#!/bin/bash
#
# encryptBin64.sh - Wrapper script for encryptBin64.pl
#
...
PERL_SCRIPT="$UTILS_DIR/edit/encryptBin64.pl"

# Verify the Perl script exists
if [ ! -f "$PERL_SCRIPT" ]; then
    echo "Error: encryptBin64.pl not found at: $PERL_SCRIPT" >&2
...
if ! command -v perl >/dev/null 2>&1; then
    ...
    echo "encryptBin64.pl requires Perl to run." >&2
    exit 1
fi
