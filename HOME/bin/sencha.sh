...
test_jvm() {
  tested_jvm=na
  test_dir=$1
  if [ -d "$test_dir/Contents/Home/jre" ] ; then
    test_dir="$test_dir/Contents/Home/jre"
  elif [ -d "$test_dir/Contents/Home" ]; then
    test_dir="$test_dir/Contents/Home"
  fi
...
if [ -z "$app_java_home" ]; then
  test_jvm "$app_home/.install4j/jre.bundle/Contents/Home/jre" 
  if [ -z "$app_java_home" ] && [ $tested_jvm = "false" ]; then
if [ -f "$db_file" ]; then
  rm "$db_file" 2> /dev/null
fi
...
cd "$app_home"
app_home=`pwd`
if [ -d "$app_home/.install4j/jre.bundle/Contents/Home/jre" ] ; then
  bundled_jre_home="$app_home/.install4j/jre.bundle/Contents/Home/jre"
elif [ -d "$app_home/.install4j/jre.bundle/Contents/Home" ]; then
  bundled_jre_home="$app_home/.install4j/jre.bundle/Contents/Home"
else
...
if [ -z "$app_java_home" ]; then
  ...
  echo The version of the JVM must be at least 1.6.0.32.
...
add_class_path "$app_home/sencha.jar"
