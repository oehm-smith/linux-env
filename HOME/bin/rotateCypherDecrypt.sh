...
# Decrypts an encrypted string by shifting each letter back by a specified number of positions.

# Function to display usage information
usage() {
    echo "Usage: $0 <string_to_decrypt> <rotation_key>"
    echo "Example: $0 'Lipps Asvph' 3"
    exit 1
}
...
for (( i=0; i<${#input_string}; i++ )); do
    ...
    if [[ "$char" =~ [a-zA-Z] ]]; then
        ...
        shifted_val=$(( (ascii_val - base_ascii - key + 26) % 26 ))
        
        # Convert the new ASCII value back to a character
        new_char=$(printf "\\$(printf '%o' $((base_ascii + shifted_val)))")
        decrypted_string+=$new_char
    else
        # If the character is not a letter, add it to the string as is
        decrypted_string+=$char
    fi
done

#echo "Encrypted string: $input_string"
