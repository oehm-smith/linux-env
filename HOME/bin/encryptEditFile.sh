#!/bin/bash
#
# encryptEditFile.sh - Wrapper script for encryptEditFile.pl
#
...
PERL_SCRIPT="$UTILS_DIR/edit/encryptEditFile.pl"

# Verify the Perl script exists
if [ ! -f "$PERL_SCRIPT" ]; then
    echo "Error: encryptEditFile.pl not found at: $PERL_SCRIPT" >&2
...
if ! command -v perl >/dev/null 2>&1; then
    ...
    echo "encryptEditFile.pl requires Perl to run." >&2
    exit 1
fi
